...
- **凍結譜面**: mp3 時代の譜面を `tools/freeze-chart.js`（実 Chromium で mp3 を解析）で
  ...
  凍結を増やすには各曲の mp3 が必要。CDN の素の URL からはもう取得できないが、
  作者としてログインすれば取得できる: `node tools/fetch-mp3s.js <保存先>`（ローカルで
  実ブラウザを開いてログイン → 全曲を {曲ID}.mp3 で自動保存）→
  `node tools/freeze-chart.js --all <保存先>` で一括凍結（1 曲なら `<曲ID> <mp3>`）。

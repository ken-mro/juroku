...
- 簾 = 左から右への掃引。右端に届いた瞬間
- 蚊 = 蚊が飛来し、大きさの変わらない中央の丸（打点）に入った瞬間。**通過後は同じ向きのまま反対側へ
  抜けていく**（遅れ 45ms で丸の縁を越え、140ms＝逃の境でパネル端近く。パネルの外には出ない）ので、
  遅れた量が見える。`paintMarker(..., late)` の `late`（0→1 = 打点からの遅れ / W_AMA）は蚊だけが使う
